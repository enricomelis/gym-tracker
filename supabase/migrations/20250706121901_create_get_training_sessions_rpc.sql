-- Migration: 20250706131001_create_get_training_sessions_rpc.sql
-- Scopo: assicurare l'esistenza della funzione get_training_sessions_rpc con la firma corretta
--        (p_athlete_id UUID DEFAULT NULL, p_start_date DATE DEFAULT NULL, p_end_date DATE DEFAULT NULL)
...
BEGIN
   ...
   * Verifica che l'atleta sia allenato dal coach corrente e restituisce le sue sessioni
   */
  ...
  /*
   * CASO 2: nessun athlete_id → restituisce tutte le sessioni degli atleti del coach corrente
   */
  ELSE
    RETURN QUERY
    SELECT ts.id,
