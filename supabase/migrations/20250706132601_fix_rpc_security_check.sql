-- Migration: Corregge il controllo di sicurezza in get_training_sessions_rpc per bypassare RLS
-- Scopo: Garantire che il controllo di autorizzazione funzioni correttamente anche in presenza di RLS

-- 1️⃣ Drop della funzione precedente
DROP FUNCTION IF EXISTS public.get_training_sessions_rpc(UUID, DATE, DATE);
-- 2️⃣ Creazione della funzione corretta
...
BEGIN
  ...
  -- ID del coach collegato all'utente corrente
  ...
  IF p_athlete_id IS NOT NULL THEN
    -- All'interno di una funzione SECURITY DEFINER, dobbiamo bypassare RLS
    -- per eseguire controlli di autorizzazione manuali sulla tabella sottostante.
